...
episode-grouped baseline prediction from existing official CSVs. See
[the frozen design](docs/analysis_1_1.md) and [release notes](docs/release_notes_1_1.md).
PyTrends export is disabled; historical backend comparisons are unvalidated and excluded.
A run may finish as a descriptive feasibility study when the prediction gate is not met.
...
The final scope is documented in GeoDemand 1.1. New data sources, acquisition backends,
dashboards and additional model families are deferred. No release or tag is published
automatically. Scientific completion is stated by each run, not inferred from software completion.
